import { MigrationInterface, QueryRunner } from "typeorm";

export class Init1766861582321 implements MigrationInterface {
    name = 'Init1766861582321'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE "user" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "email" character varying NOT NULL, "username" character varying(50) NOT NULL, "password" character varying, "google_id" character varying, "created_at" TIMESTAMP NOT NULL DEFAULT now(), "updated_at" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "UQ_e12875dfb3b1d92d7d7c5377e22" UNIQUE ("email"), CONSTRAINT "UQ_7adac5c0b28492eb292d4a93871" UNIQUE ("google_id"), CONSTRAINT "CHK_af1e74da4cf07055c5fe48a5ae" CHECK ("password" IS NOT NULL OR "google_id" IS NOT NULL), CONSTRAINT "PK_cace4a159ff9f2512dd42373760" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TYPE "public"."house_apartmenttype_enum" AS ENUM('new_build', 'resale')`);
        await queryRunner.query(`CREATE TABLE "house" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "apartment_name" character varying(30) NOT NULL, "rooms_count" integer NOT NULL, "total_area" double precision NOT NULL, "purchase_date" TIMESTAMP NOT NULL, "price" numeric(12,2) NOT NULL, "floor" integer NOT NULL, "street" character varying NOT NULL, "apartmentType" "public"."house_apartmenttype_enum" NOT NULL DEFAULT 'new_build', "created_at" TIMESTAMP NOT NULL DEFAULT now(), "updated_at" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "CHK_032a121e4caf2bf9069f766c62" CHECK ("rooms_count" > 0 AND "total_area" > 0 AND "floor" >= 0), CONSTRAINT "PK_8c9220195fd0a289745855fe908" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TYPE "public"."exchange_rates_code_enum" AS ENUM('UAH', 'USD', 'EUR', 'PLN')`);
        await queryRunner.query(`CREATE TABLE "exchange_rates" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "date" date NOT NULL, "code" "public"."exchange_rates_code_enum" NOT NULL, "rate" numeric(12,4) NOT NULL, "created_at" TIMESTAMP NOT NULL DEFAULT now(), "updated_at" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_33a614bad9e61956079d817ebe2" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE UNIQUE INDEX "IDX_807dbda9fe3a6caabbf73b8595" ON "exchange_rates" ("date", "code") `);
        await queryRunner.query(`CREATE TYPE "public"."contract_status_enum" AS ENUM('active', 'inactive')`);
        await queryRunner.query(`CREATE TYPE "public"."contract_payment_currency_enum" AS ENUM('UAH', 'USD', 'EUR', 'PLN')`);
        await queryRunner.query(`CREATE TABLE "contract" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "commencement" TIMESTAMP NOT NULL, "termination" TIMESTAMP NOT NULL, "status" "public"."contract_status_enum" NOT NULL DEFAULT 'active', "created_at" TIMESTAMP NOT NULL DEFAULT now(), "updated_at" TIMESTAMP NOT NULL DEFAULT now(), "monthly_payment" integer NOT NULL, "payment_currency" "public"."contract_payment_currency_enum" NOT NULL DEFAULT 'UAH', "houseId" uuid NOT NULL, "renterId" uuid NOT NULL, CONSTRAINT "PK_17c3a89f58a2997276084e706e8" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE "renter" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "first_name" character varying(15) NOT NULL, "last_name" character varying(20) NOT NULL, "age" integer NOT NULL DEFAULT '25', "occupied" TIMESTAMP, "vacated" TIMESTAMP, "created_at" TIMESTAMP NOT NULL DEFAULT now(), "updated_at" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_7d1963dd773c2a2a44fc93a956f" PRIMARY KEY ("id"))`);
        await queryRunner.query(`ALTER TABLE "contract" ADD CONSTRAINT "FK_71983fe8665bde9434cd63e3464" FOREIGN KEY ("houseId") REFERENCES "house"("id") ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE "contract" ADD CONSTRAINT "FK_c53f3c74676e74930dd90c6cf92" FOREIGN KEY ("renterId") REFERENCES "renter"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`);
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE "contract" DROP CONSTRAINT "FK_c53f3c74676e74930dd90c6cf92"`);
        await queryRunner.query(`ALTER TABLE "contract" DROP CONSTRAINT "FK_71983fe8665bde9434cd63e3464"`);
        await queryRunner.query(`DROP TABLE "renter"`);
        await queryRunner.query(`DROP TABLE "contract"`);
        await queryRunner.query(`DROP TYPE "public"."contract_payment_currency_enum"`);
        await queryRunner.query(`DROP TYPE "public"."contract_status_enum"`);
        await queryRunner.query(`DROP INDEX "public"."IDX_807dbda9fe3a6caabbf73b8595"`);
        await queryRunner.query(`DROP TABLE "exchange_rates"`);
        await queryRunner.query(`DROP TYPE "public"."exchange_rates_code_enum"`);
        await queryRunner.query(`DROP TABLE "house"`);
        await queryRunner.query(`DROP TYPE "public"."house_apartmenttype_enum"`);
        await queryRunner.query(`DROP TABLE "user"`);
    }

}
