...
const createContract = async (req, res) => {
  ...
  if (!renter_id || !startDate || !endDate || !monthlyPayment) {
    return res.status(HTTP_BAD_REQUEST).json({
      message:
        "Обов'язкові поля: renter_id, startDate, endDate, monthlyPayment",
  ...
  if (!ObjectId.isValid(renter_id)) {
    return res
      ...
      .json({ message: 'Невірний формат renter_id' });
  }

  if (monthlyPayment <= MIN_PAYMENT) {
    return res.status(HTTP_BAD_REQUEST).json({
  ...
  try {
    ...
    const renter = await db
      .collection('renters')
      .findOne({ _id: new ObjectId(renter_id) });
    if (!renter) {
      return res
        .status(HTTP_NOT_FOUND)
        .json({ message: 'Орендаря з таким ID не знайдено' });
    }
...
const calculateMonthsBetween = (startDate, endDate) => {
  ...
  return Math.max(MIN_MONTHS, months);
};
...
const getContracts = async (req, res) => {
  try {
    ...
    if (!validPeriods.includes(period)) {
      return res.status(HTTP_BAD_REQUEST).json({
        message:
          'Невірний період. Доступні: 1month, 6months, 1year, 5years, 10years, 15years, all',
      });
    }
    ...
    for (const contract of allContracts) {
      ...
      const adjustedStart =
        contractStart < periodStart ? periodStart : contractStart;
    ...
    filteredContracts.sort(
      (a, b) => new Date(b.adjustedStartDate) - new Date(a.adjustedStartDate)
    );
...
const getContractById = async (req, res) => {
  ...
  if (!ObjectId.isValid(contractId)) {
    return res
      .status(HTTP_BAD_REQUEST)
      .json({ message: 'Невірний формат ID договору' });
  }
  ...
  try {
    ...
    if (!contractExists) {
      return res
        .status(HTTP_NOT_FOUND)
        .json({ message: 'Договір не знайдено' });
    }
...
const updateContract = async (req, res) => {
  ...
  if (
    updateFields.monthlyPayment !== undefined &&
    updateFields.monthlyPayment <= MIN_PAYMENT
  ) {
    return res.status(HTTP_BAD_REQUEST).json({
      message: 'Місячна оплата має бути більше нуля',
    });
  ...
  if (
    updateFields.status &&
    !['active', 'completed', 'cancelled'].includes(updateFields.status)
  ) {
    return res.status(HTTP_BAD_REQUEST).json({
  ...
  try {
    ...
    if (updateFields.renter_id) {
      const renter = await db
        .collection('renters')
        .findOne({ _id: new ObjectId(updateFields.renter_id) });
      if (!renter) {
        return res
          .status(HTTP_NOT_FOUND)
          .json({ message: 'Орендаря з таким renter_id не знайдено' });
      }
    ...
    if (fieldsToUpdate.monthlyPayment) {
      fieldsToUpdate.monthlyPayment = Number.parseFloat(
        fieldsToUpdate.monthlyPayment
      );
    }
    if (fieldsToUpdate.startDate) {
      fieldsToUpdate.startDate = new Date(fieldsToUpdate.startDate);
    }
    ...
    res
      .status(HTTP_INTERNAL_ERROR)
      .json({ message: 'Не вдалося оновити договір.' });
  }
};
...
const deleteContract = async (req, res) => {
  ...
  try {
    ...
    res
      .status(HTTP_INTERNAL_ERROR)
      .json({ message: 'Не вдалося видалити договір.' });
  }
};
...
const getContractsByRenter = async (req, res) => {
  ...
  if (!ObjectId.isValid(renterId)) {
    return res
      .status(HTTP_BAD_REQUEST)
      .json({ message: 'Невірний формат ID орендаря' });
  }
